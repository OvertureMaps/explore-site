...
function readCache() {
  try {
    ...
    if (
      !parsed ||
      parsed.pmtilesUrls === null ||
      typeof parsed.pmtilesUrls !== "object" ||
      Array.isArray(parsed.pmtilesUrls) ||
      !Array.isArray(parsed.themeTypes)
    ) {
...
function writeCache(releaseUrl, releaseId, pmtilesUrls, themeTypes) {
  try {
...
/**
 * Extracts type names from a theme catalog's child links.
 * The child titles are the Overture source-layer type strings (e.g. "building", "segment").
 */
function getTypesFromCatalog(catalog) {
  ...
  if (!stacDataPromise) {
    stacDataPromise = (async () => {
      ...
      for (const { catalog, url, title } of themeCatalogs) {
        ...
        themeTypes.push({
          theme: catalog.id || title,
          types: getTypesFromCatalog(catalog),
        });
      }

      writeCache(latestReleaseUrl, releaseCatalog.id, pmtilesUrls, themeTypes);
...
/**
 * Gets the theme→type mapping derived from the STAC catalog.
 * Returns an array of { theme: string, types: string[] } in catalog order.
 * The theme IDs match Overture's schema (e.g. "buildings", "transportation").
 * The type strings are source-layer type names (e.g. "building", "segment").
 */
export async function getThemeTypes() {
  const data = await loadStacData();
  return data.themeTypes || null;
}
