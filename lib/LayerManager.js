 ...
 * With activeThemes removed, all visible layers get full colors/labels.
 */
function applyRuntimeStyle(spec, visibleTypes) {
  const meta = spec.metadata || {};
  const type = meta["overture:type"];
  const baseColor = meta["overture:color"];
  const activeColor = meta["overture:activeColor"];

  // Compute display color — always use activeColor if available
  const displayColor = activeColor || baseColor;
  const highlightColor = activeColor ? baseColor : undefined;
 ...
 */
export function addAllLayers(map, visibleTypes, mode) {
  // Geometry layers
  for (const spec of geometryLayers) {
    const styled = applyRuntimeStyle(spec, visibleTypes);
 ...
 */
export function updateLayerVisibility(map, visibleTypes) {
